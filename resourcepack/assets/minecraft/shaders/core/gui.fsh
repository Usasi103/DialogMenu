...
bool settingsFocusOutline(vec4 color) {
    ...
    if (min(size.x, size.y) > 1.5 || settingsGuiExtent.x < 474.0) return false;
    ...
    float top = min(63.0, settingsGuiExtent.y - 33.0 - min(269.0, contentHeight));
    float scrollTravel = max(0.0, 269.0 - contentHeight);
    vec2 topLeft = vec2((settingsGuiExtent.x - 474.0) * 0.5, top);
    ...
    bool horizontal = abs(max(size.x, size.y) - 474.0) < 0.15
        && settingsGuiPosition.x >= topLeft.x - 0.6 && settingsGuiPosition.x <= topLeft.x + 474.6
        && (inScrollRange(settingsGuiPosition.y, topLeft.y + 0.5, scrollTravel)
            || inScrollRange(settingsGuiPosition.y, topLeft.y + 268.5, scrollTravel));
    bool vertical = max(size.x, size.y) <= 267.15
        && settingsGuiPosition.y >= topLeft.y - scrollTravel - 0.6
        && settingsGuiPosition.y <= topLeft.y + 269.6
        && (abs(settingsGuiPosition.x - (topLeft.x + 0.5)) < 0.8
            || abs(settingsGuiPosition.x - (topLeft.x + 473.5)) < 0.8);
    return horizontal || vertical;
}

void main() {
    vec4 color = vertexColor;
    if (color.a == 0.0 || settingsFocusOutline(color)) discard;
