...
Both palettes use the same geometry and font metrics. `tools/BuildSkin.java` compiles the light palette into additional private-use glyphs in the same resource pack, so switching themes needs no resource-pack reload. Deploy the 0.1.6 JAR with its matching resource pack to include those glyphs.
...
The screen includes player information, Ambience sound and particle controls, category toggles, density dropdown, pickup notices, LootBeam settings, search, and navigation back to the main menu. Existing TrMenu menus remain available as `/settings-classic` and `/player-settings-classic`.
