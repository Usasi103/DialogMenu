...
- 将默认设置模板独立为 `demo-settings`（`Type: settings-demo`）：沿用玩家设置布局，所有开关、密度、语言、主题和缩放在单次演示内独立交互，无外部插件依赖，不写入真实偏好；重新打开重置。原服务器 `settings.yml` 与默认入口保持不变，新装默认进入演示菜单。
- 首领介绍与难度确认新增 `Canvas.MaxScale: 75`，实际画布宽度最多 414 GUI 像素，玩家较小缩放仍生效；点击区域与字体同步缩放。

- 增加按玩家保存的 50% / 75% / 100% 菜单整体缩放，设置、任务、NPC 对话与首领画布共用；背景、字体、图标与点击区域同步缩放，100% 保留原排版。
- 默认“界面与语言”新增 `Bind: menu-scale` 下拉选择；`/dmenu scale [50|75|100]` 可在菜单显示不全时直接调整。旧菜单配置默认 100%，无需重写坐标，升级需同步配套资源包。
- 使用原版 26.2 字体和点击判定验证缩放布局，补充玩家偏好隔离与保存测试。原生物品页及输入控件仍遵循客户端 GUI 比例，外部字体缺少缩放资源时画布保持原尺寸。详见 [菜单缩放](docs/guides/MENU-SCALE.md)。
