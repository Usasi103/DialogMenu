...
限流、网络错误和无效响应只在错误发生变化时输出一次说明，并在之后的 6 小时定时检查中重试；如果尚未到 GitHub 指定的退避时间，该次检查会跳过，不会提前请求或提示配置 Token。匿名 API 的限流按出口 IP 共享，启动错峰及固定 6 小时周期用于控制请求量。`enabled: false` 可完全关闭该插件的检测。
