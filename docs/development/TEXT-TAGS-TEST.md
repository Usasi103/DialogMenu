...
限制：本机没有启用 ItemsAdder，本次 IA 接入依据公开 FontImageWrapper API 实现，尚未在真实 IA 插件验证图片字符串、字体宽度及重载事件；需要持有对应版本 IA 的服务器试用。未完成游戏客户端 GPU 画面验证。大图片的基线及行高由资源定义和菜单布局共同决定。

这是独立测试产物，没有替换部署服务器 JAR，也没有创建正式版本标签或 GitHub Release。私有素材分支不在本次改动范围。
