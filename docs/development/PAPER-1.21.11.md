...
- 上述开发构建 JAR SHA-256：`ea3d44c4dc503f4513a0ed778c5eacac882c4fee9e28c2564e42d7eb20763f6f`。
...
2026-09-25 重新构建正式版并在同一独立 Paper 1.21.11 + BetterHud 测试服完成复测：

- `DialogMenu-0.1.21.jar` SHA-256：`7461475d08189848859305dcd8bb338eb23971268a2e877fdd9e0b3eff394315`。
- 完整构建、格式与框架依赖检查通过，91 项 JUnit 测试零失败、零跳过。
- 与上述开发构建相比，JAR 仅插件版本描述符及配置说明文字改变；全部运行时代码和内嵌资源 ZIP 逐字节相同。资源 ZIP SHA-256：`a40354a244a747a27decd1df203a5dc48a971768808146af242b9923690ab4d8`。
- 正式版登录后打开四个演示菜单并点击设置翻页，共收到五个 Dialog；同会话收到 387 个 BetterHud bossbar 数据包。服务端日志无 ERROR / Exception，正常执行 stop 并退出，监听端口已关闭。
- 首轮测试中协议玩家被世界内怪物击杀，死亡状态影响后续命令；给专用探针切换创造模式后重新完成上述验证。此项未修改插件代码。

Release 的 JAR 与本次实服验证文件相同；独立资源包附件直接提取自该 JAR，源码附件来自对应发布提交。
