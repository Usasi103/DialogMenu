...
- `test_server` loaded the final development JAR through its visible interactive launch console; input/output code pages are 65001 and all four deployed menus pass configuration validation. CraftEngine completed generation, validation, PackSquash and hosting. Read back all 6,766 changed hosted assets: font definitions match, PNG alpha (including invisible advance anchors) and visible colors survive optimization, and the minified focus shader is equivalent to the source. The deployed JAR matches the build's SHA-256. Existing menu content, item-source examples and custom sprite textures are preserved.
